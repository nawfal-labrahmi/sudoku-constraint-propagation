...
def naked_twins(values):
    ...
    """
    two_values_boxes = [box for box in values.keys() if len(values[box]) == 2]
    twins = [[boxA,boxB] for boxA in two_values_boxes for boxB in peers[boxA] if set(values[boxA]) == set(values[boxB])]
    for boxA, boxB in twins:
        peersA = set(peers[boxA])
        peersB = set(peers[boxB])
        peers_int = peersA.intersection(peersB)
