...
def diagonals(columns, rows):
    left_diagonal = [rows[i] + columns[i] for i in range(0, len(rows))]
    reverse_columns = '987654321'
    right_diagonal = [rows[i] + reverse_columns[i] for i in range(0, len(rows))]
